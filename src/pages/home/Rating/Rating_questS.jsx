...
const Rating = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('https://api.pps.makalabox.com/api/rating/question/get/social', {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
        setData(response.data.social);
      } catch (error) {
    ...
    const fetchUsers = async () => {
      try {
        const response = await axios.get(`https://api.pps.makalabox.com/api/rating/question/4/${selectedAward}/${selectedSubtitle}`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
          ...
          <table className="table">
            <thead>
              <tr>
                ...
                <th>ФИО</th>
                <th>Институты</th>
                <th>Воспитательная, общественная деятельност</th>
              </tr>
            </thead>
            <tbody>
              {users.map((user, i) => (
