...
const Rating = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('https://api.pps.makalabox.com/api/rating/question/get/awards', {
          headers: {
    ...
    const fetchUsers = async () => {
      try {
        const response = await axios.get(`https://api.pps.makalabox.com/api/rating/question/1/${selectedAward}/${selectedSubtitle}`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
